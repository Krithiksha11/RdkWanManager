...
)
{
    ...
    if (pDomCfg == NULL || ulInstanceNumber == 0 || ulInstanceNumber > maxDomainCount)
    {
        AnscTraceError(("%s:%d:: Invalid input\n", __FUNCTION__, __LINE__));
        return ANSC_STATUS_FAILURE;
    }

    pVirtIf = WanMgr_GetMapVirtIf_Unlocked();
    if (pVirtIf == NULL)
    {
        AnscTraceWarning(("%s: No active virtual map interface found\n", __FUNCTION__));
        return ANSC_STATUS_FAILURE;
    }

    BOOL isMapE = pVirtIf->MAP.dhcp6cMAPparameters.isMapE;
    
    pDomCfg->InstanceNumber = ulInstanceNumber;
    AnscCopyString(pDomCfg->Alias, isMapE ? "Domain_mape" : "Domain_mapt");
    pDomCfg->bEnabled = (isMapE || pVirtIf->EnableMAPT) && pVirtIf->Enable && (pVirtIf->MAP.dhcp6cMAPparameters.pdIPv6Prefix[0] != '\0');
    if (pVirtIf->PPP.Enable)
    {
        AnscCopyString(pDomCfg->WANInterface, pVirtIf->PPP.Interface);
    }
    else
    {
...
{
    DML_VIRTUAL_IFACE *pVirtIf = NULL;
    if (pDomInfo == NULL || ulInstanceNumber == 0 || ulInstanceNumber > maxDomainCount)
    {
        AnscTraceError(("%s:%d:: Invalid input\n", __FUNCTION__, __LINE__));
        return ANSC_STATUS_FAILURE;
    }
    ...
    if (pVirtIf == NULL)
    {
        AnscTraceWarning(("%s:%d:: No active virtual map interface found\n", __FUNCTION__, __LINE__));
        return ANSC_STATUS_FAILURE;
    }

    BOOL isMapE = pVirtIf->MAP.dhcp6cMAPparameters.isMapE;
    
    // Determine domain status dynamically
    if (pVirtIf->Enable)
    {
    ...
    AnscCopyString(pDomInfo->Name, isMapE ? "MAPE" : "MAPT");
    pDomInfo->TransportMode = isMapE ? WAN_DML_DOMAIN_TRANSPORT_MAPE : WAN_DML_DOMAIN_TRANSPORT_MAPT;
    pDomInfo->IfaceStatus = (pVirtIf->Status == WAN_IFACE_STATUS_UP) ? WAN_DML_INTERFACE_STATUS_Up : WAN_DML_INTERFACE_STATUS_Down;

    AnscCopyString(pDomInfo->IfaceName, isMapE ? "ip6tnl" : "map0");
...
)
{
    ...
    //Call updated function to refresh rule data into pEntry
    ANSC_STATUS ret = WanDmlMapDomGetRule_Data(ulMapDomInstanceNumber, ulIndex, pEntry);
    if (ret != ANSC_STATUS_SUCCESS)
    {
        AnscTraceError(("%s:%d:: WanDmlMapDomGetRule_Data failed for domain %lu rule %lu\n",
                        __FUNCTION__, __LINE__, ulMapDomInstanceNumber, ulIndex));
        return ret;
    }
...
{
    DML_VIRTUAL_IFACE *pVirtIf = NULL;
    if (pMapRule == NULL || ulMapDomInstanceNumber == 0 || ulMapDomInstanceNumber > maxDomainCount)
    {
        AnscTraceError(("%s:%d:: Invalid input\n", __FUNCTION__, __LINE__));
        return ANSC_STATUS_FAILURE;
    }
